    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 bg-gray-50 rounded-xl border border-dashed">
          <IconHeartBroken className="w-16 h-16 text-gray-300 mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-gray-600">Danh sách trống</h3>
          <p className="text-sm text-gray-400 mt-1 mb-6">Bạn chưa lưu sản phẩm nào vào danh sách yêu thích.</p>
          <Button asChild>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {items.map((item) => {
            ...
            return (
              <div key={item.id} className="group border rounded-xl overflow-hidden hover:shadow-md transition-all bg-white flex flex-col">
                <Link href={`/product/${p.id}`} className="relative aspect-square overflow-hidden bg-gray-100 block">
                  <img 
                    src={p.image_url} 
                    alt={p.name}
