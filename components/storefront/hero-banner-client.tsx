  ...
  return (
    <section className="relative min-h-[calc(100vh-5rem)] w-full bg-secondary overflow-hidden flex flex-col select-none group">
          ...
          >
            ...
            <Image
              ...
              className={`object-contain object-center transform transition-all duration-[8000ms] ease-out ${
                idx === safeIndex ? "scale-115" : "scale-100"
              }`}
      ...
      <div className="relative z-20 flex-1 w-full max-w-7xl mx-auto px-4 md:px-10 flex flex-col justify-center py-20 my-auto">
        <div key={currentBanner.id} className="w-full lg:w-2/3 animate-[fade-in_0.7s_cubic-bezier(0.16,1,0.3,1)] space-y-6 sm:space-y-7">
          {/* Badge */}
          ...
          {/* Title */}
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold text-white leading-[1.12] tracking-tight whitespace-pre-line drop-shadow-sm">
            {currentBanner.title}
          </h1>

          {/* Description */}
          {currentBanner.description && (
            <p className="text-base sm:text-lg lg:text-xl text-gray-200 max-w-xl line-clamp-3 font-normal opacity-90 leading-relaxed">
              {currentBanner.description}
            </p>
          )}
